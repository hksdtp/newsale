...
export function HRManagement() {
  ...
  const fetchEmployeesAndTeams = async () => {
    try {
      ...
      // Check if NHÓM 5 exists
      const team5Exists = teamsData.some(team => team.name && team.name.includes('NHÓM 5'));
      setHasTeam5(team5Exists);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gray-800 rounded-lg border border-gray-700 p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400 mt-1">Quản lý thông tin nhân viên, vai trò và phòng ban</p>
          </div>
      ...
      {/* Database Setup Guide - Show if NHÓM 5 doesn't exist */}
      {!hasTeam5 && user?.role === 'retail_director' && (
        <DatabaseSetupGuide onSetupComplete={fetchEmployeesAndTeams} />
      )}

      {/* Content based on active tab */}
      {activeTab === 'management' && user?.role === 'retail_director' ? (
